# Capital report — daily deposits into Total Capital (2026-09-23)

**Definitions.** *Total Capital (live)* = principal $1900.00 + realized $2404.63 + open mark-to-market $307.61 = **$4612.24** — the Overview's own stat, so it moves with marks and can fall without a trade closing. *Daily deposit* = the capital added on a local calendar day = that day's **booked** PnL (finished trades, booked at `closedAt ?? resolvedAt`) + any **ledger injection**. This report and the chart on `/capital` are the same `src/lib/capital.ts` series.

## Window

- Window: **2026-08-25 → 2026-09-23** (30 local days)
- Opening booked capital: **$1737.80** (`opening` ledger entry $1900.00 dated 2026-07-22)
- Deposits in window: **+$2566.83** = booked +$2566.83 + injected +$0.00
- Closing booked capital: **$4304.63** · peak close $4304.63 on 2026-09-23
- Up days 14 · down days 13 · flat 3 · longest up-run 3 (current 3)
- Open positions excluded from every row: notional $443.30, unrealized +$307.61

## Daily deposits (same numbers as the /capital chart)

| day | deposit | booked | closing |
|---|---|---|---|
| 2026-09-23 | +$19.90 | +$19.90 | $4304.63 |
...
npx tsx scripts/capital-deposits-report.ts --days=30 --write  # + drafts/capital-deposits-2026-09-23.md
