# Capital report — daily deposits into Total Capital (2026-09-24)

**Definitions.** *Total Capital (live)* = principal $1900.00 + realized $2380.01 + open mark-to-market $298.51 = **$4578.52** — the Overview's own stat, so it moves with marks and can fall without a trade closing. *Daily deposit* = the capital added on a local calendar day = that day's **booked** PnL (finished trades, booked at `closedAt ?? resolvedAt`) + any **ledger injection**. This report and the chart on `/capital` are the same `src/lib/capital.ts` series.

## Window

- Window: **2026-08-26 → 2026-09-24** (30 local days)
- Opening booked capital: **$1737.80** (`opening` ledger entry $1900.00 dated 2026-07-22)
- Deposits in window: **+$2542.21** = booked +$2542.21 + injected +$0.00
- Closing booked capital: **$4280.01** · peak close $4300.26 on 2026-09-23
- Up days 14 · down days 14 · flat 2 · longest up-run 3 (current 0)
- Open positions excluded from every row: notional $628.36, unrealized +$298.51

## Daily deposits (same numbers as the /capital chart)

| day | deposit | booked | closing |
|---|---|---|---|
| 2026-09-24 | -$20.25 | -$20.25 | $4280.01 |
...
npx tsx scripts/capital-deposits-report.ts --days=30 --write  # + drafts/capital-deposits-2026-09-24.md
