...
2. **Wallet CSV export** (`/export-polymarket-trades`): paste any public wallet → per-fill CSV (`timestamp_utc, type, market_title, market_slug, condition_id, outcome, side, shares, price, usd_size, cash_flow_usd, transaction_hash`), free preview, download needs a free account. Useful for **spot backfill and validation of individual wallets** and for reconstructing a wallet's history without our data-api path (where 429s are the recurring cost). Caveat found after the first pass: their `robots.txt` **disallows `/export-polymarket-trades/`** to crawlers, so this is a *you-in-the-UI* tool — not something we automate, and not something I drive at scanner scale.
